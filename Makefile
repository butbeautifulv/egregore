.PHONY: dev dev-infra dev-api dev-ui dev-worker dev-workers dev-docker dev-langfuse dev-langfuse-fresh langfuse-dev-setup dev-obs dev-tool-gateway obs-reload
...
dev-worker:
	uv run egregore worker --daemon --idle-timeout 0
...
dev-workers:
	...
	for i in $$(seq 1 $$replicas); do \
	  ...
	  uv run egregore worker --daemon --idle-timeout $$idle & \
	done; \
	wait
...
dev-tool-gateway:
	uv run uvicorn interfaces.gateways.tool.server:create_app --factory --host 0.0.0.0 --port 8090
