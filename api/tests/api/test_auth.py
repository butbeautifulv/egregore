...
@pytest.mark.unit
def test_api_auth_ingress_success(auth_settings, monkeypatch):
    from interfaces.api.app import create_app
    ...
    monkeypatch.setattr(
        "interfaces.control_plane.critic_service.get_critic_service",
        lambda: SimpleNamespace(register=lambda: None),
    )
    monkeypatch.setattr(
        "interfaces.control_plane.coordinator_service.get_coordinator_service",
        lambda: SimpleNamespace(register=lambda: None),
    )
