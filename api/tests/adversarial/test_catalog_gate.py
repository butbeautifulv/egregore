...
def test_draft_skill_blocked_on_load(monkeypatch):
    ...
    monkeypatch.setattr("cys_core.infrastructure.skill.load_skill.get_use_dynamic_catalog", lambda: True)
    monkeypatch.setattr("cys_core.infrastructure.skill.load_skill.get_skill_catalog", lambda: catalog)
