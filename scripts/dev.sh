...
(cd backend && PROMETHEUS_MULTIPROC_DIR="$MPDIR" uv run egregore serve --port 8080) &
PIDS+=($!)

if wait_for_redis; then
  for i in $(seq 1 "$REPLICAS"); do
    echo "[dev] starting worker $i/$REPLICAS"
    (cd backend && PROMETHEUS_MULTIPROC_DIR="$MPDIR" uv run egregore worker --daemon --idle-timeout "$IDLE") &
    PIDS+=($!)
  done
else
