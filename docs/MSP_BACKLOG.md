...
1. **Model Gateway is now covered by the Release Gate security suites.** `fe870ec` adds the
   ...
   `999d2bd` added targeted guard-path cases. Release Gate `30014193942` completed green across
   the Model Gateway lint, adversarial, domain-coverage, unit, architecture, build, and image-scan legs.
2. **Docker `ExecutionBackend` bootstrap is now explicit and opt-in.** `ecdb964` adds the pinned
   ...
   Release Gate `30014310150` failed only its dispatcher Trivy image scan; Compose configuration
   itself was already validated and no backend container was run locally.
3. **Model Gateway now has an opt-in Helm workload.** `56a88b6` adds its ClusterIP Service and
   ...
   Release Gate `30014533490` hit the same dispatcher Trivy image scan; no cluster was changed.
