...
| `domain-coverage` | The actual CI job (`release-gate.yml`'s `domain-coverage`) runs, inline, in `backend/contracts`: `pytest tests/domain/ -q --cov=src/cys_core/domain --cov-fail-under=100` — **no `--include` narrowing**, the 100% threshold applies to the *whole* `cys_core/domain` tree. There is a **separate, narrower** local convenience target, `make domain-gate` → `./scripts/pytest_batches.sh tests/domain --cov --domain-gate`, whose 100% threshold only covers `coverage report --include="src/cys_core/domain/{runs,catalog,observability}/*"`. These are two genuinely different checks — the Makefile target is a local dev convenience, not what blocks merges; conflating them previously produced a wrong "already covered" conclusion in `docs/MICROSERVICES_SPLIT_PLAN.md` §16.10-16.11 (corrected there after verifying the exact CI command directly). `domain-gate` **only exists for `backend/contracts`** — `cys_core/domain` physically lives only there; `worker`/`api` install it as an editable path dependency, so a `coverage --include=src/cys_core/domain/*` run inside either always reports "No data to report" regardless of actual coverage (verified 2026-07-17 by actually running `make -C backend/worker domain-gate`, which failed every time before the Makefile targets were removed — see `Makefile`/`backend/{worker,api}/Makefile`). **Known stale**: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `docs/UV_WORKFLOW.md`, `docs/DEVELOPMENT.md`, `docs/REFACTOR_COMPLETE.md` still show `--domain-gate` being run per-package across all three, or reference the retired `ci.yml`/pre-split single-package layout — not fixed this round, flagged for a dedicated pass. |
