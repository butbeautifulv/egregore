...
`api/.env` раньше repo-root `.env` — но только когда оба файла реально существуют и
...
словить эту гонку, если когда-нибудь `api/.env` и repo-root `.env` разойдутся — стоит
