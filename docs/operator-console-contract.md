...
| `critic:{engagement_id}` | control job | Critic verdict |
| `coordinator:{engagement_id}` | control job | Final report summary |
...
| GET | `/v1/work-orders?tenant_id&limit` | список work orders (preferred) |
| POST | `/v1/work-orders` | старт: `{ goal?, intake?, profile_id?, tenant_id? }` |
| GET | `/v1/work-orders/{id}?tenant_id` | detail work order |
| POST | `/v1/work-orders/{id}/follow-ups` | operator follow-up (preferred) |
| GET | `/v1/work-orders/{id}/follow-ups?tenant_id` | follow-up transcript |
...
| `tool_start` / `tool_done` / `tool_error` | если `features.stream_agent_tools` |
...
| `tools` | array | `{ name, status, tool_call_id? }` |
| `streaming` | bool | |
