...
def patch_catalog(monkeypatch, catalog: InMemoryAgentCatalog) -> None:
    ...
    Unlike the api/worker conftest.py copies of this helper, contracts has no
    bootstrap.container of its own (only api and worker have a composition
    root by that name) — nothing here targets it.
    """
    monkeypatch.setattr(
...
def _reset_container_and_memory_catalog(monkeypatch: pytest.MonkeyPatch) -> None:
    """Use in-memory catalog and reset shared infra singletons between tests.

    No bootstrap.container reset here: contracts has no Container of its own
    (only api and worker each have their own composition root) — there is no
    equivalent singleton to clear.
    """
    ...
    # agents/ is a sibling of contracts/worker/api under backend/, not inside
    # any one service's own directory — settings.agents_root defaults to the
    # relative string "agents", which only resolved correctly against CWD
    # back when the monolith's own directory contained it directly. Setting
    # the AGENTS_ROOT env var alone isn't enough: bootstrap.settings.settings
    # (imported directly by cys_core.registry.product_context.default_agents_root
    # and others) is a module-level singleton built once at import time, long
    # before this fixture's env vars take effect — patch its attribute too.
    monkeypatch.setenv("AGENTS_ROOT", str((Path.cwd().parent / "agents").resolve()))
    monkeypatch.setattr(settings, "agents_root", str((Path.cwd().parent / "agents").resolve()))
    ...
    class _AgentsRoot:
        def agents_root(self):
            # agents/ is a sibling of contracts/worker/api under backend/,
            # not inside any one service's own directory — pytest runs with
            # CWD set to this service's own directory, so go one level up.
            return Path.cwd().parent / "agents"
