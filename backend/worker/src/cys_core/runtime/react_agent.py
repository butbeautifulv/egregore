...
from cys_core.application.runtime_config import get_stream_agent_token_streaming
...
from cys_core.infrastructure.observability.egress_streaming_callback import build_egress_streaming_callbacks
    ...
    async def arun(
        ...
        *,
        ...
        if stream_context is not None:
            invoke_config["callbacks"] = build_egress_streaming_callbacks(stream_context)
        ...
        try:
            ...
            for _ in range(recursion_limit or self._max_iterations):
                if stream_context is not None and get_stream_agent_token_streaming():
                    response = None
                    async for chunk in bound_model.astream(messages, config=invoke_config):
                        if isinstance(chunk, AIMessage):
                            response = chunk
                    if response is None:
                        response = await bound_model.ainvoke(messages, config=invoke_config)
                else:
                    response = await bound_model.ainvoke(messages, config=invoke_config)
                ...
                for call_index, call in enumerate(tool_calls):
                    ...
                    messages.append(
                        ToolMessage(
                            content=content,
                            tool_call_id=tool_call_id_from_mapping(call, index=call_index),
                        )
                    )
