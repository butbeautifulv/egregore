    ...
    def get_worker_orchestrator(self, persona: str | None = None):
        if persona not in self._worker_orchestrators:
            ...
            if backend_kind == "in_process":
                # dispatcher physically no longer carries cys_core/runtime or
                # cys_core/middleware (docs/MICROSERVICES_SPLIT_PLAN.md §1) — those moved to
                # backend/agent-runtime. in_process execution would need the agent runtime
                # in this same process, which no longer exists here; fail clearly instead of
                # letting the import fail deep inside a lazy call.
                raise NotImplementedError(
                    "execution_backend='in_process' is not supported in backend/dispatcher — "
                    "the agent runtime lives in backend/agent-runtime now. Use 'subprocess', "
                    "'k8s', or 'docker' (see docs/MICROSERVICES_SPLIT_PLAN.md §1)."
                )
