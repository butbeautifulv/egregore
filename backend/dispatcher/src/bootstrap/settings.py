...
class Settings(BaseSettings):
    ...
    # "in_process" is structurally unsupported in this package — cys_core/runtime and
    # cys_core/middleware live in backend/agent-runtime now (docs/MICROSERVICES_SPLIT_PLAN.md §1).
    # Defaults to "subprocess" here (worker/agent-runtime's own settings.py copies keep
    # "in_process" as their default — this is a deliberate divergence, not drift).
    execution_backend: str = Field(default="subprocess", validation_alias="EXECUTION_BACKEND")
    # Path to the agent-runtime package's own venv python (e.g.
    # backend/agent-runtime/.venv/bin/python) — SubprocessExecutionBackend spawns this
    # interpreter instead of dispatcher's own sys.executable, since dispatcher's venv no longer
    # has the langchain/langgraph/deepagents dependencies agent-runtime's `run-sandboxed-job`
    # CLI command needs. Empty by default (falls back to sys.executable, which will fail loudly —
    # see subprocess_backend.py — until this is actually deployed alongside agent-runtime).
    agent_runtime_python_executable: str = Field(
        default="", validation_alias="AGENT_RUNTIME_PYTHON_EXECUTABLE"
    )
