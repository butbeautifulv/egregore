...
class Container:
    ...
    def wire_runtime(self) -> None:
        """Not called in backend/dispatcher (see get_container() below) — configures a
        callback on cys_core.runtime.agent, which lives in backend/agent-runtime now
        (docs/MICROSERVICES_SPLIT_PLAN.md §1). Kept, unused, for structural parity with
        worker/agent-runtime's identical copy of this file."""
        from cys_core.runtime.agent import configure_runtime_memory_reader  # ty: ignore[unresolved-import]
...
def get_container() -> Container:
    ...
    # container.wire_runtime() intentionally not called — configures
    # cys_core.runtime.agent, which lives in backend/agent-runtime now (docs/
    # MICROSERVICES_SPLIT_PLAN.md §1).
