...
def test_worker_cli_commands(monkeypatch, capsys):
    ...
    # cmd_agent (interactive single-agent CLI) is not exercised here — it needs
    # cys_core.runtime.agent, which dispatcher no longer carries (docs/
    # MICROSERVICES_SPLIT_PLAN.md §1); that command stays covered in
    # backend/agent-runtime's identical test.
