...
def test_registry_helpers_and_temp_agent_loading(tmp_path, monkeypatch):
    ...
    # Normally wired by Container.__init__ (api/worker only) at startup —
    # contracts has no Container of its own, so this test wires the loader
    # itself instead of relying on ambient state left behind by another test.
