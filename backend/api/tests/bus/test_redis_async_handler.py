...
async def test_redis_async_handler_uses_main_loop(monkeypatch):
    ...
    try:
        ...
        # This test's event loop closes when the test ends — leaving the
        # module-level main-loop pointer set to it would make any later
        # test's _run_async_handler try to schedule onto a closed loop.
        set_bus_main_event_loop(None)
