...
class KafkaJobQueue:
    ...
    async def _ensure_producer(self) -> bool:
        ...
        try:
            ...
            self._producer = await start_with_retry(_build, source="kafka_queue_producer")
    ...
    async def _ensure_consumer(self) -> bool:
        ...
        try:
            ...
            self._consumer = await start_with_retry(_build, source="kafka_queue_consumer")
            return True
        except Exception as exc:
            self._connected = False
    ...
    async def adequeue(self, timeout: float = 0.0) -> WorkerJob | None:
        ...
        while time.monotonic() < deadline:
            ...
            if self._matches_persona(job):
                return job
            # Not an error: this worker is persona-scoped and picked up a job
