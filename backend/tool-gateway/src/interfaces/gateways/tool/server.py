...
from interfaces.gateways.tool.auth import GatewayAuthError, require_gateway_role
...
async def _route(method: str, path: str, headers: dict[str, str], body: bytes) -> tuple[int, bytes, str]:
    ...
    if method == "GET" and path == "/metrics":
        payload = await asyncio.to_thread(generate_metrics_payload)
        return 200, payload, "text/plain; version=0.0.4; charset=utf-8"

    if method == "POST" and path == "/invoke":
        try:
            auth = await require_gateway_role(headers.get("authorization"))
        ...
        try:
            request = ToolInvokeRequest.model_validate(data)
...
async def _build_response(reader: asyncio.StreamReader) -> tuple[int, bytes, str]:
    ...
    try:
        ...
        logger.exception("tool gateway request failed", extra={"path": path})
...
async def start_server(host: str = "0.0.0.0", port: int = 8092) -> asyncio.base_events.Server:
...
async def serve_forever(host: str = "0.0.0.0", port: int = 8092) -> None:
