...
import { Dialog, DialogContent, DialogTitle } from "@/vendor/gui/ui/dialog"
    ...
    <>
      ...
      >
        {attachments.map((attachment) => (
          <div
            key={attachment.id}
            className="group relative aspect-square overflow-hidden rounded-gui-muted border bg-muted"
          >
            <button
              type="button"
              className="size-full"
        ...
        <DialogContent className="max-w-4xl p-2">
          ...
          {preview && (
            <div className="flex flex-col gap-3">
              <div className="relative max-h-[80vh] w-full overflow-hidden rounded-gui-muted">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={preview.viewUrl}
              ...
              <div className="flex min-w-0 flex-wrap items-center justify-between gap-2 px-2 pb-2">
                <OverflowText className="min-w-0 text-sm">
