...
Vendored artifacts used by contract tests in `api/tests/contracts/`. These files are **not** deployed from this repo directly; they mirror upstream cxado manifests so CI can run without the meta-repo checkout.
...
3. Run `cd api && uv run pytest tests/contracts/test_kafka_topics_k8s.py -q`
